...
class SMAStrategy(StrategyBase):
    ...
    def __init__(
        ...
        # 存储上一周期的均线值用于检测交叉
        self._prev_sma: Dict[str, Dict[str, Optional[float]]] = {}
    ...
    def _calculate_sma(self, prices: Deque[float], window: int) -> Optional[float]:
        """计算简单移动平均线
    ...
    def get_current_sma(self, symbol: str) -> Dict[str, Optional[float]]:
        """获取当前均线值"""
        if symbol not in self._price_history:
